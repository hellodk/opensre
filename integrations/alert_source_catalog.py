...
_ROUTING_TABLE: dict[str, AlertSourceRouting] = {
    ...
    "keycloak": routing(("keycloak",), ("keycloak",)),
    "nginx": routing(("nginx",), ("nginx",)),
...
_ALIASES_TABLE: dict[str, tuple[str, ...]] = {
    ...
    "airflow": ("airflow", "dag"),
    "kafka": ("kafka",),
    "keycloak": ("keycloak", "sso", "oidc", "openid", "login failed", "locked out", "realm"),
    "rabbitmq": ("rabbitmq", "amqp"),
    "nats": ("nats", "jetstream", "nats-server", "slow consumer"),
    "nginx": ("nginx", "openresty", "bad gateway", "gateway timeout"),
    "supabase": ("supabase",),
