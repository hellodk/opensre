...
<Note>
**Credential resolution.** Every Telegram delivery surface — the scheduler, `/watchdog`, and `/watch` — resolves the bot token the same way: integration store first, then `resolve_env_credential("TELEGRAM_BOT_TOKEN")` (process env, then the credentials file). Chat id is non-secret: `--chat-id` → store `default_chat_id` → `TELEGRAM_DEFAULT_CHAT_ID` env (plain `os.getenv`).
</Note>
...
Watchdog alerts and cron deliveries still use the outbound-only paths — they do not require the gateway process.
...
Delivery test — trip a one-shot watchdog alarm:

```bash
sleep 30 &
opensre watchdog --pid $! --max-runtime 2s --once --provider telegram
```

An alarm message should appear in the configured chat within a few seconds. Long messages are truncated to Telegram's 4,096-character limit.
