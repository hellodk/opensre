...
OpenSRE's Rocket.Chat integration delivers messages — agent-requested posts, watchdog alarms, scheduled reports — to any channel your user or bot account can post to. Useful for teams running self-hosted or cloud Rocket.Chat workspaces.
...
For a real delivery test, trip a one-shot watchdog alarm (see [Watchdog alarms](#watchdog-alarms) above). Long messages are truncated to 4,096 characters.
