...
There are no registered Discord investigation tool packages today — Discord is delivery + gateway chat.
...
| **`/investigate` missing** | Re-run `opensre onboard` / setup to re-register commands. Global commands can take up to an hour to propagate. |
