...
<Note>
Steps 1–3 give you **outbound delivery** (watchdog alarms and agent-requested messages posted to a channel). To `@mention` the agent from a Buzz channel and get replies, also do [Step 4](#step-4-enable-two-way-chat).
</Note>
...
Trip a one-shot watchdog alarm:

```bash
sleep 30 &
opensre watchdog --pid $! --max-runtime 2s --once --provider buzz
```

An alarm message should appear in the configured channel within a few seconds. Long messages are truncated to 4,096 characters.
