...
OpenSRE queries Azure Monitor Log Analytics through the public Query REST API during alert investigations. Each query is bounded by a `take` clause so result sets stay within a safe row limit.
