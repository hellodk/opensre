...
OpenSRE works with many model providers. You choose which one with
`LLM_PROVIDER`. Where a provider supports both API key and OAuth, use
`LLM_AUTH_METHOD` to pick the auth method.

Default model IDs are defined in
[`config/config.py`](https://github.com/Tracer-Cloud/opensre/blob/main/config/config.py).
Routing is in
...
| --- | --- | --- |
| Anthropic API key | `LLM_PROVIDER=anthropic`<br />`LLM_AUTH_METHOD=api_key`<br />`ANTHROPIC_API_KEY` | Reasoning: `claude-opus-4-7`<br />Toolcall: `claude-haiku-4-5-20251001` |
| OpenAI API key | `LLM_PROVIDER=openai`<br />`LLM_AUTH_METHOD=api_key`<br />`OPENAI_API_KEY` | Reasoning: `gpt-5.4-mini`<br />Toolcall: `gpt-5.4-mini` |
...
| Provider | Config | Default models |
| --- | --- | --- |
| Anthropic OAuth | `LLM_PROVIDER=anthropic`<br />`LLM_AUTH_METHOD=oauth`<br />Onboarding runs `claude auth login` | Claude Code CLI default (both roles) |
| OpenAI OAuth | `LLM_PROVIDER=openai`<br />`LLM_AUTH_METHOD=oauth`<br />OpenSRE opens `localhost:1455` for Codex-compatible OAuth | Codex CLI default (both roles) |
| Google Gemini CLI | `LLM_PROVIDER=gemini-cli`<br />`gemini` login or API key env | Gemini CLI default (both roles) |
| Google Antigravity CLI | `LLM_PROVIDER=antigravity-cli`<br />`agy` browser OAuth / OS keyring | Antigravity CLI configured model (both roles) |
...
If a provider supports more than one auth method, onboarding asks for the
provider first, then the auth method. Example: choose **Anthropic**, then
**OAuth** for a Claude subscription, or **API key** to paste
`ANTHROPIC_API_KEY`. The provider value stays `anthropic` or `openai`;
`LLM_AUTH_METHOD=oauth` selects the OAuth path.

Browser login, token storage, refresh, and logout are handled by the vendor
CLI. OpenSRE runs the onboarding UI and does not store OAuth tokens itself.

In the interactive shell, `/model` lists common models. Some providers also
...
Use `opensre auth` to log in without writing secrets to `.env`:

| Command | What it does |
| --- | --- |
| `opensre auth` | Show auth status for subscription and API-key providers |
| `opensre auth login deepseek` | Guide DeepSeek setup, validate `DEEPSEEK_API_KEY`, store it in the system keychain, and select DeepSeek |
| `opensre auth login claude` | Set up Claude Code CLI subscription login |
| `opensre auth login chatgpt` | Run OpenSRE-managed ChatGPT OAuth for the Codex path |
...
`opensre auth login` does not read browser cookies, profiles, local storage, or
IndexedDB. API-key providers use a hidden paste prompt and keyring storage.
OpenAI OAuth uses OpenSRE’s local Codex-compatible callback server. Other
subscription providers use the vendor CLI for browser login.

`opensre auth` and `/auth status` are safe to show in prompts: they do not read
API keys from Keychain. For API-key providers they check environment variables
and non-secret metadata in `~/.opensre/llm-auth.json`. If you delete a key from
Keychain directly, status may look stale until you run
`opensre auth verify <provider>` or make a request. Verification marks the
provider `stale` when the secret is gone.

For Codex, status checks do not run `codex login status` by default — some Codex
versions open a browser during that check. To refresh login, run
`/login chatgpt` or `opensre auth login chatgpt` in an interactive terminal.
OpenSRE starts a temporary callback server on
`http://localhost:1455/auth/callback`, exchanges the OAuth code, writes
Codex-compatible tokens, then redirects to the Codex-style
`/success?id_token=...` page. If `/success` already includes token material,
OpenSRE stores that instead of dropping the callback. Use `codex login` only as
a direct CLI fallback.
...
Defaults in `config/config.py` are US cross-region inference profile IDs for
...
OpenAI OAuth tokens are stored by OpenSRE in Codex-compatible format. Other CLI
providers authenticate with the vendor’s own login command.
...
<AccordionGroup>
<Accordion title="OpenAI OAuth backend">
...
Requires the [OpenAI Codex CLI](https://github.com/openai/codex). If
`CODEX_MODEL` is unset, OpenSRE omits `-m` so `codex exec` uses the CLI’s
configured model. If `CODEX_BIN` is unset, the binary is resolved from `PATH`
and known install locations. Run `opensre onboard`, `/login chatgpt`, or
`opensre auth login chatgpt` to start OpenSRE-managed Codex browser login on
`localhost:1455` and save `LLM_PROVIDER=openai` with `LLM_AUTH_METHOD=oauth`.
Existing `LLM_PROVIDER=codex` configs still work.

</Accordion>

<Accordion title="Anthropic OAuth backend">

```bash
export LLM_PROVIDER=anthropic
export LLM_AUTH_METHOD=oauth
# Authenticate through onboarding, `/login claude`, or the Claude Code CLI:
claude auth login
# Optional overrides (all blank by default):
...
(`npm i -g @anthropic-ai/claude-code`). If `CLAUDE_CODE_MODEL` is unset, OpenSRE
omits `--model` and the CLI uses its default. If `CLAUDE_CODE_BIN` is unset, the
binary is resolved from `PATH` and known install locations. Run
`opensre onboard`, `/login claude`, or `opensre auth login claude` to launch
Claude browser login when needed and save `LLM_PROVIDER=anthropic` with
...
- Investigation tool-calling adapters:
  [Investigation tool calling](/investigation-tool-calling)
- API provider guide:
