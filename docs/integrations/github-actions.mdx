...
This is **not** a separate integration id — it uses the same GitHub MCP credentials as [GitHub](/github). For mutating PR CI fixes, see [Fix GitHub PR CI](/github-ci-fix).
...
- These tools are read-only; mutating CI fixes go through [GitHub PR CI fix](/github-ci-fix) with explicit confirmation.
