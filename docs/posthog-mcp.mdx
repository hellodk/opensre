...
OpenSRE connects to PostHog’s hosted [Model Context Protocol (MCP)](https://posthog.com/docs/model-context-protocol) server. During investigations the agent can call PostHog products — product analytics, feature flags, error tracking, experiments, surveys, and HogQL — through MCP tools.

This is separate from the [PostHog REST integration](/posthog), which only stores project credentials for the REST API. The onboarding wizard lists **PostHog** (REST) and **PostHog (MCP)** as separate choices. Configure REST with `opensre integrations setup posthog`.
