...
6. If files changed, it commits only the fix-run changes and pushes the PR branch or fresh repair branch.
...
If no checks are failing, OpenSRE says so and does not push. It does **not** open a new PR. Merged or closed PRs are refused — if the failure already landed on the base branch, ask for the branch itself ("fix the CI on main").
