...
2. **Product analytics**: forwarded to PostHog as an `$ai_generation` event
   ...
   usage and quality can be tracked. Investigation turns also include the
   model/provider/token usage of the investigation run and an `investigation_id`
   join key; failed turns carry structured error properties (`$ai_is_error`,
