...
**Quick Access** duplicates frequently used commands (`/integrations`, `/model`, `/health`, `/watch`, `/status`, `/help`) for faster discovery in the picker.
...
Trust mode skips execution confirmation prompts for **elevated** commands (`/watch`, `/cancel`, `/uninstall`, integration remove, etc.).
...
<Tip>
  During watchdog demos, `/trust on` avoids confirmation fatigue. Turn it off before running destructive commands you might mistype.
</Tip>
...
| Elevated | `/watch`, `/cancel`, `/integrations remove`, `/fleet kill`, `/uninstall` | Prompt unless trust on |
