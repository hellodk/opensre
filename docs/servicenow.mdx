...
Select **ServiceNow** under *Incident & Comms* and provide instance URL, username,
and password. The wizard validates credentials with a one-row authenticated read of
the `sys_user` table before saving.
...
`SERVICENOW_PASSWORD` is also resolved from the OS keyring when the environment
...
of `sys_user` (for example `itil`). Prefer the onboarding wizard so the password
stays in the OS keyring instead of `.env`.
...
- Prefer the onboarding wizard so the password stays in the OS keyring.
