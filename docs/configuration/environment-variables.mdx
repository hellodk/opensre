...
`opensre onboard` or `opensre integrations setup` are written to an
owner-only file at `~/.opensre/credentials.json` (mode `0600`). OpenSRE
does not write new secrets to the OS keychain, so first-run setup does
not trigger a keychain prompt.

A value in the process environment or a local `.env` file wins over the
local file. Keep `.env` out of source control and never commit real secrets.

Secrets stored in the OS keychain by an earlier OpenSRE version are copied
into that local file once, then the keychain copies are removed.
