"""Tests for the Keycloak realm overview tool."""
...
class TestKeycloakRealmOverviewToolContract(BaseToolContract):
    def get_tool_under_test(self):
        return get_keycloak_realm_overview.__opensre_registered_tool__


def test_metadata() -> None:
    rt = get_keycloak_realm_overview.__opensre_registered_tool__
    assert rt.name == "get_keycloak_realm_overview"
...
def test_run_happy_path() -> None:
    fake_result = {"source": "keycloak", "available": True, "users_total": 4}
    with patch(
        "integrations.keycloak.tools.keycloak_realm_overview_tool.get_realm_overview",
