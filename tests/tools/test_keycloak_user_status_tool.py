"""Tests for the Keycloak user status tool."""
...
class TestKeycloakUserStatusToolContract(BaseToolContract):
    def get_tool_under_test(self):
        return get_keycloak_user_status.__opensre_registered_tool__


def test_metadata() -> None:
    rt = get_keycloak_user_status.__opensre_registered_tool__
    assert rt.name == "get_keycloak_user_status"
    assert rt.source == "keycloak"
    assert rt.injected_params == _KEYCLOAK_INJECTED
    assert set(rt.public_input_schema.get("properties") or {}) == {"username"}
    assert "username" in (rt.public_input_schema.get("required") or [])
...
def test_run_happy_path() -> None:
    fake_result = {"source": "keycloak", "available": True, "found": True}
    with patch(
        "integrations.keycloak.tools.keycloak_user_status_tool.get_user_status",
...
def test_full_path(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _install(
        monkeypatch,
        {
            ...
            f"{REALM_PATH}/attack-detection/brute-force/users/{ALICE_ID}": (
