"""Tests for the Keycloak login failures tool."""
...
class TestKeycloakLoginFailuresToolContract(BaseToolContract):
    def get_tool_under_test(self):
        return get_keycloak_login_failures.__opensre_registered_tool__


def test_metadata() -> None:
    rt = get_keycloak_login_failures.__opensre_registered_tool__
    assert rt.name == "get_keycloak_login_failures"
    assert rt.source == "keycloak"
    assert rt.injected_params == _KEYCLOAK_INJECTED
    assert set(rt.public_input_schema.get("properties") or {}) == {
        "event_type",
        "max_events",
        "username",
    }
...
def test_run_happy_path() -> None:
    fake_result = {"source": "keycloak", "available": True, "events": []}
    with patch(
        "integrations.keycloak.tools.keycloak_login_failures_tool.get_login_failures",
...
def test_full_path(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert result["summary"]["lockouts"] == 1
